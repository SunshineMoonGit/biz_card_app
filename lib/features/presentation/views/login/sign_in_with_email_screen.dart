...
class SignInWithEmailScreen extends ConsumerStatefulWidget {
  static String get route => '/auth_select/sign_in_with_email';

  const SignInWithEmailScreen({super.key});

  @override
  ConsumerState<SignInWithEmailScreen> createState() => _SignInWithEmailScreenState();
}
...
class _SignInWithEmailScreenState extends ConsumerState<SignInWithEmailScreen> {
  ...
  void initState() {
    super.initState();
    manager = ControllerManager(type: ControllerType.signIn);
  }
    ...
    return BasicLayout(
      body: Form(
        ...
        child: CustomScrollView(
          slivers: [
            // 앱바
            const AuthWithEmailAppBar(title: AppString.signIn),

            // 입력란
            AuthWithEmailTextField(controllers: manager.controllers),

            // 추가 이동 (회원가입/로그인)
            const AuthWithEmailToRoute(cType: ControllerType.signIn),
          ],
        ),
      ),
      // 제출
